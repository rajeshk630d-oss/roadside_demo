...
function get_setting($key = NULL){
 ...
 function exportData($assocDataArray,$fileName,$heading)
{
	//echo '<pre>';print_r($assocDataArray);die;
    ob_clean();
   
   
   
    header('Content-Type: text/csv');
    header("Content-Transfer-Encoding: utf-8-sig");
    header('Content-Disposition: attachment;filename=' . $fileName);///die('hello');
